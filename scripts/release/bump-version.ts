...
export function bumpVersion(
  version: Version,
  options: BumpVersionOptions = { type: "patch" }
): Version {
  ...
  if (
    ...
    !PRE_RELEASE_NAMES.includes(options.preReleaseName)
  ) {
    throw new InvalidPreReleaseNameError(options.preReleaseName);
  }

  try {
    const [rawVersionNumber, rawPreReleaseId] = version.split("-") as [
      VersionNumber,
    ...
    return `${rawVersionNumber}-${bumpPreReleaseId(
      rawPreReleaseId,
      options.preReleaseName!
    )}`;
  } catch (e) {
...
function bumpPreReleaseId(
  id: PreReleaseId | undefined,
  nextName: PreReleaseName
): PreReleaseId {
