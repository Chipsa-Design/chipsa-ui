...
export type ObjectPosition = [ObjectPositionValue] | [ObjectPositionValue, ObjectPositionValue];
...
export class ObjectPlacement {
    ...
    calc({ object, container, fit = 'fill', position = ['center'] }: CalculationParams) {
        container ||= this.container;
        const { width, height } = parseObjectFit(object, container, fit);
        const { x, y } = parseObjectPosition({ width, height }, container, position);
        return { x, y, width, height };
    }
}

function parseObjectFit(object: Dimensions, container: Dimensions, fit: ObjectFit = 'fill') {
    ...
    switch (fit) {
        ...
        case 'scale-down':
            ...
            if (scaleDownWidth <= containerWidth && scaleDownHeight <= containerHeight) {
...
function parseObjectPosition(object: Dimensions, container: Dimensions, position: ObjectPosition) {
